...
초기 가중치: ID 변수 모으기 0.97, ChangeEvent 명시 import 1.00, 폼 상태/핸들러 일원화 1.01, 초기값 상수 분리 0.92
...
[ 3] ChangeEvent 명시 import  (미시도) 점수: -3 -> 개선됨
[ 4] ID 변수 모으기              (미시도) 점수:  1 -> 개선됨
[ 5] ID 변수 모으기              (가중치) 점수:  1 -> 변화없음 (정체 1/3)
[ 6] 초기값 상수 분리              (가중치) 점수:  1 -> 변화없음 (정체 2/3)
[ 7] ChangeEvent 명시 import  (가중치) 점수:  1 -> 변화없음 (정체 3/3)

>> 모든 fix 시도 후 3회 연속 정체. 멈춤.
